...
- Removed duplicate `SCRIPT_DIR` assignment in `archive-url.sh`. The variable is now set once at the top, before relay discovery sourcing.

### Changed

- Updated Nostr profile `about` text to "Tag me and give me a URL and I'll archive it."
...
- `deploy-nsite.sh` in the `naan-archiver` skill. Every node can now deploy its own browseable archive index as an nsite. The script clones the site from GitHub, builds it, and deploys using the node's nsec. Supports `--update` to pull latest and redeploy.
...
- Requester attribution. Archive events now include a `p` tag with a `requester` marker when triggered by someone (via public mention or DM). The index site resolves the requester's profile and displays "by NAAN, requested by Gigi" style attribution on archive cards.
- `--requester <pubkey>` flag on `archive-url.sh`, passed through to kind 4554 and NIP-71 events.
- `monitor-mentions.sh` now passes the mention author's pubkey as `--requester`.
